...
class FlightsController < ApplicationController
  ...
  def filter
    if params[:password] == ENV['POST_PASSWORD']
      ...
      to_where = passed_params["to"] == 0 ? "arrival_airport_id > ?" : "arrival_airport_id = ?"
      ...
      if passed_params["type"] == "Epic"
        flights = Flight.where("shortcut = ? AND cheapest_price > ? AND #{to_where} AND departure_time >= ? AND arrival_time <= ? AND epic = ?", true, 0, to, start_date, end_date, true).order(sort).paginate(:page => passed_params["page"], :per_page => 10)
      else
        flights = Flight.where("shortcut = ? AND cheapest_price > ? AND #{to_where} AND departure_time >= ? AND arrival_time <= ?", true, 0, to, start_date, end_date).order(sort).paginate(:page => passed_params["page"], :per_page => 10)
      end
